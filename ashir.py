...
def plot_bar_datay(data, countries, years, labelx="", labely = "Rate"):
    ...
    barWidth = 15/(len(countries)  * (2*len(years)))
    
    fig = plt.subplots(figsize =(14, 6)) # create a figure of size 14x6
    ...
    plt.xlabel(labelx, fontweight ='bold', fontsize = 11)
    plt.ylabel(labely, fontweight ='bold', fontsize = 11)

    # Adding xTicks
    plt.xticks([r + barWidth for r in range(len(countries))], countries , rotation=25)
...
if __name__ == "__main__":
    ...
    # Reading the data into the DataFrames
    ...
    df_urban_population["World"] = df_urban_population.mean(axis=1) # Adding a row for Average for the whole world
    ...
    #Plot a line graph of any urban population
    plot_line(df_urban_population, countries=countries, years=[str(1980 +i) for i in range(40)])


    #Plotting the correlation heat map

    #Get the whole data for Pakistan
